...
void avoid_cliff(int dirct){                                      //input is that it measured something and which direction to go to
  while(1){
    ...
    while(measured == 0){                                         //measuring loop (doesn't enter on first iteration)
      ...
      if(measured == 0 && check1 == 1 &&(distance-dist_capt) >= set_distance*6){                           //if it went 2*set_distance without encountering cliff
    ...
    while(distance - dist_capt <= set_distance){delay(10);}
